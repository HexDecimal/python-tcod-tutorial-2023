...
class LogRenderer:
    ...
    def init(cls, world: tcod.ecs.Registry, width: int, height: int) -> Self:
        ...
        self.y_position = self.max_y_pos
        return self

    def scroll(self, y_dir: int, wrap: bool = False) -> None:
        """Scroll the log by the amount provided by y_dir."""
        self.y_position = max(0, min(self.y_position + y_dir, self.max_y_pos))
    ...
    @property
    def max_y_pos(self) -> int:
        """Maximum value of y_position."""
        return max(0, self.log_height - self.height)
